...
namespace FUNAttendanceAndPayrollSystemClient.Controllers.Staff
{
    public class AttendanceController : Controller
    {
        ...
        public async Task<IActionResult> ShowTimeSheet(int? EmployId = null)
        {
            string baseUrl = _configuration["ApiUrls:Base"];
            using var client = new HttpClient();
            ...
            if (role == "staff")
            {
                ...
                employeeIdToUse = EmployId ?? sessionEmployeeId ?? 0;
            }
            else
            {
                employeeIdToUse = sessionEmployeeId ?? 0;
            }

            string url = $"{baseUrl}/Employee/MyAttendance?emp={employeeIdToUse}";
            var response = await client.GetAsync(url);
            var strData = await response.Content.ReadAsStringAsync();
